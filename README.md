...
This project was built collaboratively. All three contributors participated across every layer of the application.

| Contributor | Areas |
|---|---|
| **Murilo** | React architecture, custom hooks (`useCMEData`, `useFilters`, `useSupabase`), Supabase Edge Function (`donki-proxy`), CI/CD pipeline, Vercel deployment |
| **Yasmin Cardozo** | Dashboard page, CME components (`CMECard`, `CMEList`, `CMEStats`, `CMECharts`), data visualization with Recharts, API service layer |
| **Guilherme Adami** | UI design system, CSS styling, History and About pages, responsive layout, error handling components |
